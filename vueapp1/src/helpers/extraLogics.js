 ...
 *
 * for some currency symbol is not available in Intl.NumberFormat so made it static
 */

export const currencyFormatter = (value, currency = "USD", lang = "en") => {
    const symbol = getCurrencySymbol(currency);

    // Determine the position of the symbol based on the language
    const symbolOnRight = ['ar', 'vi', 'kr', 'zh-hans', 'zh-hant'].includes(lang);

    // Format the value with the currency symbol
    const formattedValue = symbolOnRight ? `${value}${symbol}` : `${symbol}${value}`;
            ...
            if(value[filterOptions[i].section[j].name]){
                ...
                if(filterOptions[i].section[j].isCustom) {
                    ...
                    if(fieldType === 'date'){
                        ...
                        if (Array.isArray(value[filterOptions[i].section[j].name])) {
                            ...
                            params += filterOptions[i].section[j].name + '=' + value[filterOptions[i].section[j].name].id + '&';
                        }
                    }
                }
            }
                ...
                if(filterOptions[i].section[j].isCustom) {
                    ...
                    if(fieldType === 'date'){
                        ...
                        if (Array.isArray(value[filterOptions[i].section[j].name])) {
                            ...
                            params[filterOptions[i].section[j].name] = value[filterOptions[i].section[j].name]; // Assign it directly or handle as needed
                        }
                    }
                }
            }
...
export const scrollToElement = (id, offset = 0) => {
    ...
    if (targetElement.length) {
        // Element exists, proceed with the scroll
        $('html, body').animate(
...
export const fetchDataBasedOnUrl = (apiUrl,urlParams, axiosConfig, successCallback, errorCallback) => {
    ...
    axios.get(apiUrl, { params: urlParams }).then(res => {
        ...
        res.data.data.forEach(item => {
            ...
            transformedObject[key] = value_meta ? value_meta : item.value;
        });
